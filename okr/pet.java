...
public class pet {
    ...
    public boolean trainPet(int id, String command) {
        try {
            ...
            if (((PetRep) petRepository).getCommandsById(id, 1).contains(command))
                view.showMessage("это мы уже умеем");
            else {
                if (!((PetRep) petRepository).getCommandsById(id, 2).contains(command))
                    view.showMessage("невыполнимая команда");
                else {
                    ((PetRep) petRepository).train(id, command);
    ...
    public void getCommands(int id) {
        try {
            view.printAll(((PetRep) petRepository).getCommandsById(id, 1), String.class);
        } catch (RuntimeException e) {
            view.showMessage(e.getMessage());
        }
    }
